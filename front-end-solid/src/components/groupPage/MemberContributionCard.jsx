...
export default function MemberContributionCard() {
  ...
  const randomProfileDark = colorsDark[Math.floor(Math.random() * colorsDark.length)];

  console.log("randomProfile", colors.indexOf(randomProfile));
    ...
    <div>
      <AccordionBox>
        <AccordionMemberBox expandIcon={<ExpandMoreIcon />} aria-controls="panel1a-content" id="panel1a-header">
          ...
          <Typography sx={{ marginInlineStart: "10px" }}>Member Name </Typography>
          <Typography sx={{ marginInlineStart: "10px" }}>200 USD </Typography>
        </AccordionMemberBox>
      </AccordionBox>
    </div>
  );
